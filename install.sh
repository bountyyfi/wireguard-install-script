...
#
# WireGuard Installer
# Supports: Ubuntu 22.04+, Debian 11-13, AlmaLinux/Rocky 9+, CentOS 9+, Fedora
#
# No third-party dependencies or external connections
...
if grep -qs "ubuntu" /etc/os-release; then
	...
	# Detect Debian 13 Trixie (testing) by codename
	if grep -qi 'trixie' /etc/debian_version 2>/dev/null || grep -qi 'trixie' /etc/os-release 2>/dev/null; then
		os_version="13"
		is_trixie="1"
	fi
...
if [[ "$os" == "debian" ]]; then
	# Allow Debian 13 Trixie even if it contains /sid
	if grep -q '/sid' /etc/debian_version && [[ "$is_trixie" != "1" ]]; then
...
# Check for kernel module support (required - no userspace fallback)
if systemd-detect-virt -cq && ! grep -q '^wireguard ' /proc/modules; then
	echo "This system is running in a container without the WireGuard kernel module.
Please ensure the WireGuard kernel module is loaded on the host system,
or run this installer on a system with native WireGuard support."
	exit
fi
...
if [[ ! -e /etc/wireguard/wg0.conf ]]; then
	...
	if echo "$ip" | grep -qE '^(10\.|172\.1[6789]\.|172\.2[0-9]\.|172\.3[01]\.|192\.168)'; then
		...
		echo "(You can find your public IP by running: curl -4 ifconfig.me)"
